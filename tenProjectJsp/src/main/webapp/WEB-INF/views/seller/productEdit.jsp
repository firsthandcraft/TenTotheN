...
<title>product view seller</title>
<!-- 공통 -->
...
<main>
        <div class="box">
        	<p class="title">상품</p>
            <form id = "editProductForm"action="${pageContext.request.contextPath}/seller/productEdit" method="post">
                <table class="enroll">
                    <tbody>
                    	<tr>
                    		<th>상품번호</th>
                        ...
                        <tr>
                            <th>상품명</th>
                            <td><input type="text" name="p_name" id=""value="<c:out value="${p.p_name}"/>"></td>
                        </tr>
                        <tr>
                            <th>이미지</th>
                            <td>
                            	<img alt="productimg" name="p_img" src="${p.p_img}">
                            	<input type="file" name="p_img" value="${p.p_img}">
                            </td>
                        </tr>
                        <tr>
                            <th>상품설명</th>
                            <td><textarea name="p_content" cols="50" rows="10"> ${p.p_content}</textarea></td>
                        </tr>
                        <tr>
                            <th>진열여부</th>
                            <td>
                            	 <c:if test="${p.p_permission=='허가'}"><input type="checkbox"name="p_display" <c:if test="${p.p_display=='N'}">selected</c:if>></c:if>
                            	<c:if test="${p.p_permission!='허가'}"><span>상품 심사 허가 후에 진열여부를 설정하실 수 있습니다.</span></c:if>
	                       </td>
                        </tr>
                        <tr>
                            <th>수량</th>
                        ...
                        <tr>
                        	...
                        	<td class="p_permission">
                        		<c:if test="${p.p_permission=='심사중'}"><input type= "text" name="p_permission" readonly value='심사중'></c:if>
                            		
                            		<c:if test="${p.p_permission=='허가'}"><input type= "text" name="p_permission" readonly value='허가'></c:if>
                            		<c:if test="${p.p_permission=='반려'}"><input type= "text" name="p_permission" readonly value='반려'></c:if>
               ...
               <section class="submitBtnWrap">
                    <button><a class="listBtn" href="${pageContext.request.contextPath}/seller/productList">목록으로</a></button>
               		 <c:if test="${p.p_permission=='허가'}"><button class="addBtn" type="button">수정</button></c:if>
               </section>
            </form>
        </div>
    </main> 
