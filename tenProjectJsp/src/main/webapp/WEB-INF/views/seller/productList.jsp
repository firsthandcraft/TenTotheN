...
<main>
        <div class="box">
            <div class="categoryWrap">
                ...
                <form action="${pageContext.request.contextPath}/seller/productSearch"method="get">
                    <input type="text"name="p_name">
                    <button><span class="material-symbols-outlined searchBtn">search</span></button>
                </form>
            </div>
            <div class="btnWrap">
                <a class="addBtn" href="${pageContext.request.contextPath}/seller/productAdd"><span class="material-symbols-outlined">add</span>등록</a>
                ...
                <tbody>
           		<c:forEach var="p" items="${list}" varStatus="status">
					  <tr>
                        ...
                        <td>${p.p_category}</td>
                        <td><a href="${pageContext.request.contextPath}/seller/productDetail?p_idx=${p.p_idx}">${p.p_name}</a></td>
                        <td>${p.p_quantity}</td>
                        <td>${p.p_price}</td>
                        <td class="displayBtn">
...
//$('.tableList').on("click", "td:first-child input[type='checkbox']",function(){
	...
	if(checked){	
		 var chekedbox= $(this).parents("tr").find('.p_idx').text();
		categoryArr.push(chekedbox);
		
		console.log($("td:first-child input[type='checkbox']"));
